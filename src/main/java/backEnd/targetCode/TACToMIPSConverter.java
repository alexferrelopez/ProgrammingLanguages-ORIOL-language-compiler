...
public class TACToMIPSConverter implements TargetCodeGeneratorInterface {
	...
	private String convertTACInstruction(TACInstruction instruction) throws IOException {
		return switch (instruction.getOperator()) {
			...
			// *** Binary Operations ***
			case "GT", "LT", "EQ", "NEQ", "OR", "AND" -> showOperation(instruction, assignmentOperations.addPendingOperation(instruction.getOperand1(), instruction.getOperand2(), instruction.getResult(), instruction.getOperator()));
